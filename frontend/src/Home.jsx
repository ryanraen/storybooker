...
import { Box, Container, Typography, Button, Card, CardContent, Grid, AppBar, Toolbar } from '@mui/material';
  ...
  return (
    <Box>
        ...
        <Toolbar sx={{ justifyContent: 'space-between' }}>
          ...
          <Box>
            {['Features', 'How It Works', 'Get Started'].map((item) => (
              <Button key={item} color="inherit" href={`#${item.toLowerCase().replace(/ /g, '-')}`}>
                {item}
              </Button>
            ))}
          </Box>
      ...
      <Container maxWidth="lg" sx={{ py: 10, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: 'center', gap: 5 }}>
        <Box flex={1}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Button variant="contained" size="large">Create Your Storybook</Button>
            <Button variant="outlined" size="large">See Examples</Button>
          </Box>
        </Box>
        <Box flex={1}>
      ...
      <Box sx={{ py: 10, bgcolor: '#3f51b5', color: '#fff', textAlign: 'center' }} id="cta">
        ...
        <Button variant="contained" color="secondary" size="large">Start Creating Now</Button>
      </Box>
